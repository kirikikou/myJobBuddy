...
class AwsServiceDecider {
  constructor() {
    ...
    this.lambdaMaxDuration = 15 * 60 * 1000;
    this.lambdaMemoryLimit = 3008;
    this.fargateMinDuration = 5 * 60 * 1000;
  }

  async decideService(url, jobTitles = [], options = {}) {
    loggingService.service('AwsServiceDecider', 'service-decision-start', { url });
    
    if (!this.isAwsEnabled) {
      loggingService.service('AwsServiceDecider', 'aws-disabled', { message: 'using local processing' });
      return {
        service: 'local',
    ...
    try {
      ...
      loggingService.service('AwsServiceDecider', 'decision-made', { service: decision.service, reason: decision.reason });
      return decision;
      
    } catch (error) {
      loggingService.error('AWS service decision error', { url, error: error.message });
      return {
        service: 'local',
  ...
  estimateDuration(profile, service) {
    if (!profile) {
      return service === 'lambda' ? 300000 : 600000;
    }
  ...
  async getBatchDecisions(urls, jobTitles = [], options = {}) {
    loggingService.service('AwsServiceDecider', 'batch-decisions-start', { urlCount: urls.length });
    ...
    for (const url of urls) {
      try {
        ...
        loggingService.error('Error deciding service for URL', { url, error: error.message });
        decisions[url] = {
          service: 'local',
