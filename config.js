...
const ENV_SCHEMA = {
  ...
  timeouts: {
    TO_REQUEST_MS: { type: 'integer', min: 1000, max: 120000, default: 30000 },
    TO_NAV_MS: { type: 'integer', min: 1000, max: 120000, default: 20000 },
    TO_STEP_MS: { type: 'integer', min: 100, max: 60000, default: 5000 },
    TO_GLOBAL_MS: { type: 'integer', min: 1000, max: 600000, default: 60000 },
    TO_IDLE_MS: { type: 'integer', min: 1000, max: 300000, default: 10000 },
    BACKOFF_BASE_MS: { type: 'integer', min: 100, max: 10000, default: 1000 },
    BACKOFF_MAX_MS: { type: 'integer', min: 1000, max: 60000, default: 30000 },
    CAPTCHA_COOLDOWN_MS: { type: 'integer', min: 0, max: 300000, default: 60000 },
    IDEMPOTENCY_TTL_MS: { type: 'integer', min: 1000, max: 300000, default: 10000 },
    MEM_CACHE_DEFAULT_TTL_MS: { type: 'integer', min: 1000, max: 3600000, default: 30000 }
  },
  retries: {
    RETRIES_MAX: { type: 'integer', min: 0, max: 10, default: 3 },
    RETRYABLE_ERRORS: { type: 'string', default: 'TIMEOUT,NETWORK,DNS,CONNECTION' },
    RETRY_JITTER: { type: 'boolean', default: true },
    RETRY_FACTOR: { type: 'number', min: 1, max: 5, default: 2 }
  },
  cache: {
    CACHE_ENABLED: { type: 'boolean', default: true },
    CACHE_FRESH_S: { type: 'integer', min: 0, max: 604800, default: 86400 },
    CACHE_RETENTION_D: { type: 'integer', min: 0, max: 365, default: 31 },
    CACHE_MAX_MB: { type: 'integer', min: 0, max: 10240, default: 1024 },
    CACHE_NS: { type: 'string', default: 'myjobbuddy' },
    CACHE_EVICTION: { type: 'enum', values: ['LRU', 'LFU'], default: 'LRU' },
    CACHE_DPROFILE_TTL_D: { type: 'integer', min: 0, max: 365, default: 30 }
  },
  polling: {
  ...
  requests: {
    RL_GLOBAL_PS: { type: 'integer', min: 0, max: 1000, default: 10 },
    RL_DOMAIN_PM: { type: 'integer', min: 0, max: 100, default: 30 },
    RATE_LIMIT_WINDOW_MS: { type: 'integer', min: 100, max: 60000, default: 1000 },
    ROBOTS_RESPECT: { type: 'boolean', default: true },
    CRAWL_DELAY_MS: { type: 'integer', min: 0, max: 10000, default: 1000 },
  ...
  search: {
    SEARCH_FUZZY_THRESHOLD: { type: 'integer', min: 50, max: 100, default: 80 }
  },
  flags: {
...
class SecureConfig {
  ...
  runStartupCleanup() {
    ...
    const excludePatterns = this._config.startup.exclude.map(pattern => {
    ...
    const cleanDirectory = (dirPath, folderName, mode) => {
            ...
            try {
              ...
              if (stats.isFile()) {
                ...
                if (cutoffTime === 0 || stats.mtime.getTime() < cutoffTime) {
                  candidates.push({ path: itemPath, sizeMB: fileSizeMB, type: 'file' });
                }
      ...
      if (candidates.length === 0) {
        this.smartLog('buffer', `${folderName}: ${folderFiles} files, none eligible for cleanup`);
      ...
      for (const candidate of candidates) {
        ...
        if (mode === 'dry-run') {
          this.smartLog('batch', `DRY-RUN would delete: ${path.relative(this._config.paths.rootDir, candidate.path)} (${candidate.sizeMB.toFixed(2)}MB)`);
          deletedCount++;
          deletedSize += candidate.sizeMB;
        } else if (mode === 'confirm' && !this._config.startup.confirmAccept) {
          this.smartLog('buffer', `${folderName}: skipping ${candidates.length} files (CLEAN_CONFIRM_ACCEPT=false)`);
          break;
          ...
          try {
            ...
            deletedCount++;
            deletedSize += candidate.sizeMB;
            this.smartLog('timing', `Deleted: ${path.relative(this._config.paths.rootDir, candidate.path)} (${candidate.sizeMB.toFixed(2)}MB)`);
          } catch (deleteError) {
      ...
      if (deletedCount > 0) {
        try {
          const removeEmptyDirs = (dirPath) => {
            ...
            const nonExcludedItems = items.filter(item => {
              const itemPath = path.join(dirPath, item);
              return !isExcluded(itemPath);
            });
            
            if (nonExcludedItems.length === 0 && dirPath !== this._config.paths.rootDir) {
              const parent = path.dirname(dirPath);
              if (parent !== this._config.paths.rootDir) {
                fs.rmdirSync(dirPath);
                this.smartLog('timing', `Removed empty directory: ${path.relative(this._config.paths.rootDir, dirPath)}`);
          ...
          this.smartLog('fail', `Error removing empty directories: ${cleanupError.message}`);
        }
      }

      const action = mode === 'dry-run' ? 'would delete' : 'deleted';
      this.smartLog('batch', `${folderName}: ${folderFiles} files scanned, ${deletedCount} ${action} (${deletedSize.toFixed(2)}MB)`);
    ...
    const folderMappings = [
      ...
      { name: 'profiles', path: this._config.paths.profilesDir, mode: this._config.startup.modes.profiles },
      { name: 'monitoring_data', path: this._config.paths.monitoringDir, mode: this._config.startup.modes.monitoring_data }
    ];

    this.smartLog('batch', 'Startup cleanup initiated');
    ...
    this.smartLog('timing', `Startup cleanup completed: ${totalDeleted}/${totalFiles} files processed, ${totalSizeMB.toFixed(2)}MB freed (${duration}ms)`);
  }
