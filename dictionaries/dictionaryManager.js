...
class DictionaryManager {
  constructor() {
    ...
    this._useOptimizedPatterns = true;
  }
  ...
  _getScrapingPatterns(language) {
    if (!this._useOptimizedPatterns) {
      return this._getLegacyScrapingPatterns(language);
    }

    const cacheKey = `scraping-optimized-${language}`;
    if (this._cache.has(cacheKey)) {
      return this._cache.get(cacheKey);
    }

    try {
      const ScrapingPatternMerger = require('./scraping/merger');
      const patterns = ScrapingPatternMerger.getPatterns(language);
      
      if (ScrapingPatternMerger.validatePatterns(patterns)) {
        this._cache.set(cacheKey, patterns);
        config.smartLog('cache', `Optimized scraping patterns loaded for ${language}`);
        return patterns;
      } else {
        config.smartLog('fail', `Invalid optimized patterns for ${language}, falling back to legacy`);
        return this._getLegacyScrapingPatterns(language);
      }
    } catch (error) {
      config.smartLog('fail', `Error loading optimized patterns for ${language}: ${error.message}`);
      return this._getLegacyScrapingPatterns(language);
    }
  }

  _getLegacyScrapingPatterns(language) {
    const cacheKey = `scraping-legacy-${language}`;
    if (this._cache.has(cacheKey)) {
      return this._cache.get(cacheKey);
    }

    try {
      const patterns = require(`./scraping/${language}`);
      this._cache.set(cacheKey, patterns);
      config.smartLog('cache', `Legacy scraping patterns loaded for ${language}`);
      return patterns;
    } catch (error) {
      if (language !== this.fallbackLang) {
        config.smartLog('fail', `Error loading legacy patterns for ${language}, trying ${this.fallbackLang}`);
        return this._getLegacyScrapingPatterns(this.fallbackLang);
      }
      config.smartLog('fail', `Critical: Cannot load patterns for ${language}: ${error.message}`);
      return {};
    }
  }
  ...
  get(category, subcategory = null) {
    ...
    try {
      ...
      if (category === 'complexSites') {
        ...
        const langDict = this._getScrapingPatterns(this.currentLang);
        ...
        if (!result && this.currentLang !== this.fallbackLang) {
          const fallbackDict = this._getScrapingPatterns(this.fallbackLang);
          result = subcategory ? 
  ...
  getPaginationSelectors() {
    try {
      ...
      return [
        ...(universal.paginationSelectors || []),
        ...progressive.level1
      ];
    } catch (error) {
      const progressive = this.getProgressive('pagination');
      return progressive.level1 || [];
    }
  }
  ...
  setOptimizedMode(enabled = true) {
    if (this._useOptimizedPatterns !== enabled) {
      this._useOptimizedPatterns = enabled;
      this.clearCache();
      config.smartLog('cache', `Optimized patterns mode: ${enabled ? 'enabled' : 'disabled'}`);
    }
  }

  isOptimizedMode() {
    return this._useOptimizedPatterns;
  }
  ...
  getAllAvailableData() {
    ...
    try {
      ...
      return {
        ...
        optimizedMode: this._useOptimizedPatterns,
  ...
  getDictionaryForLanguage(language) {
    ...
    return new DictionaryInstance(finalLang, this._cache, this._useOptimizedPatterns);
  }
...
class DictionaryInstance {
  constructor(language, cache, useOptimized = true) {
  ...
  get(category, subcategory = null) {
    ...
    try {
      ...
      if (category === 'complexSites') {
        ...
        const langDict = this._getScrapingPatterns(this.language);
        const universal = require('./core/universal');
        const platforms = require('./core/platforms');
        const patterns = require('./core/patterns');
