(function (root, factory) {
  if (typeof define === 'function' && define.amd) {
    ...
    module.exports = factory(require('./config.client.js'));
  } else {
    root.UIManager = factory(root.clientConfig);
  }
  ...
  class UIManager {
    ...
    getNavigationTranslations() {
      if (!this.isInitialized) {
        config.smartLog('fail', 'UIManager not initialized, returning empty navigation');
        return {};
      }
    
      try {
        const navigation = this.translate('navigation');
        ...
        if (navigation === 'navigation') {
          config.smartLog('fail', 'Navigation translation key not found, using fallback');
          return this._getFallbackNavigationTranslations();
        }
        
        return {};
      } catch (error) {
        config.smartLog('fail', `Error getting navigation translations: ${error.message}`);
        return this._getFallbackNavigationTranslations();
      }
    }
    
    _getFallbackNavigationTranslations() {
    ...
    async _performInitialization() {
      try {
        ...
        await this._validateTranslations();
    ...
    async _validateTranslations() {
      const requiredKeys = ['navigation', 'common', 'messages'];
      const currentLang = this.currentLanguage;
      
      if (!this.translations[currentLang]) {
        config.smartLog('fail', `No translations loaded for ${currentLang}`);
        return;
      }
      
      const missing = [];
      for (const key of requiredKeys) {
        if (!this.translations[currentLang][key]) {
          missing.push(key);
        }
      }
      
      if (missing.length > 0) {
        config.smartLog('fail', `Missing translation keys for ${currentLang}: ${missing.join(', ')}`);
        
        if (currentLang !== this.fallbackLanguage && this.translations[this.fallbackLanguage]) {
          config.smartLog('langue', `Copying missing keys from fallback language ${this.fallbackLanguage}`);
          for (const key of missing) {
            if (this.translations[this.fallbackLanguage][key]) {
              this.translations[currentLang][key] = this.translations[this.fallbackLanguage][key];
            }
          }
        }
      } else {
        config.smartLog('win', `All required translation keys validated for ${currentLang}`);
      }
    }
    ...
    async _waitForUserData(){
      ...
      const a=typeof u.maxAttempts==='number'?u.maxAttempts:(config.i18n.retry&&config.i18n.retry.max);
      const d=typeof u.intervalMs==='number'?u.intervalMs:(config.i18n.retry&&config.i18n.retry.delayMs);
      const e=Math.max(1,typeof u.logEvery==='number'?u.logEvery:a);
      config.smartLog('langue','Checking userData availability');
      for(let i=0;i<a;i++){
        if(window.userData&&typeof window.safeSaveUserPreferences==='function'){config.smartLog('win',`userData available after ${i}`);return;}
        if(i%e===0)config.smartLog('langue',`Waiting for userData. attempt ${i}/${a}`);
        await new Promise(r=>setTimeout(r,d));
      }
      config.smartLog('fail',`userData not available after ${a} attempts`);
    }
