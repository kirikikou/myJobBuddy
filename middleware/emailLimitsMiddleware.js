...
const loadUserPreferences = async (userId) => {
  try {
    ...
    console.error(`Error loading user preferences for ${userId}:`, error.message);
    return null;
  }
};
...
const saveUserPreferences = async (userId, userData) => {
  try {
    ...
    console.error(`Error saving user preferences for ${userId}:`, error.message);
    return false;
  }
};
...
const updateEmailSearchUsage = async (userId, liveSearchUsed = 0, cacheSearchUsed = 0) => {
  try {
    ...
    if (success) {
      console.log(`[EMAIL-LIMITS] Updated usage for user ${userId}: +${liveSearchUsed} live, +${cacheSearchUsed} cache`);
    }

    return success;
  } catch (error) {
    console.error(`Error updating email search usage for ${userId}:`, error.message);
    return false;
  }
};
