...
const loadUserPreferences = async (userId) => {
  try {
    ...
    loggingService.error('Error loading user preferences', { userId, error: error.message });
    return null;
  }
};
...
const saveUserPreferences = async (userId, userData) => {
  try {
    ...
    loggingService.error('Error saving user preferences', { userId, error: error.message });
    return false;
  }
};
...
const updateEmailSearchUsage = async (userId, liveSearchUsed = 0, cacheSearchUsed = 0) => {
  try {
    ...
    if (success) {
      loggingService.service('EmailLimits', 'usage-updated', { userId, liveSearchUsed, cacheSearchUsed });
    }

    return success;
  } catch (error) {
    loggingService.error('Error updating email search usage', { userId, error: error.message });
    return false;
  }
};
