...
class ErrorCaptureMiddleware {
  constructor() {
    ...
    if (this.enabled) {
      console.log(`🔍 ErrorCaptureMiddleware ENABLED - Sampling: ${this.samplingRate * 100}%`);
      this.setupProcessHandlers();
    } else {
      console.log('🔕 ErrorCaptureMiddleware DISABLED - Performance mode');
    }
  }
  ...
  logError(errorData) {
    ...
    try {
      console.error('[ERROR_CAPTURE]', JSON.stringify({
        ...errorData,
        sampled: true,
        rate: this.samplingRate
      }));
    } catch (e) {
      console.error('[ERROR_CAPTURE] Failed to log error:', e.message);
    }
  }
