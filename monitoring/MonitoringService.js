...
class MonitoringService {
  ...
  async loadHistoricalData() {
    try {
      ...
      console.log(`📈 Loaded monitoring data: ${this.domainStats.size} domains, ${this.userStats.size} users`);
    } catch (error) {
    ...
    try {
      ...
      for (const period of periods) {
        try {
          ...
          console.log(`📊 Loaded ${period} metrics: ${this.metrics[period].size} entries`);
        } catch (fileError) {
          console.log(`No existing ${period}_metrics.json, starting fresh`);
        }
      }
    } catch (error) {
      console.warn(`Failed to load aggregated metrics: ${error.message}`);
    }
  }
  ...
  startMetricsCollection() {
    ...
    console.log('🔄 Started realtime metrics collection (10s interval)');
  }
  ...
  startDataPersistence() {
    ...
    console.log('💾 Started data persistence (60s interval)');
  }
  ...
  async persistMetrics() {
    try {
      ...
      console.log('💾 Monitoring metrics persisted (including historical data)');
    } catch (error) {
