...
class MonitoringService {
  ...
  async initialize() {
    ...
    loggingService.service('MonitoringService', 'initialized');
  }
  ...
  async ensureDirectories() {
    ...
    for (const dir of dirs) {
      try {
        ...
        loggingService.error('Failed to create directory', { dir, error: error.message });
      }
    }
  }
    ...
    try {
      ...
      for (const file of files) {
        try {
          ...
          loggingService.service('MonitoringService', 'file-not-found', { file });
        }
      }
      
      loggingService.win('Monitoring data loaded', { domains: this.domainStats.size, users: this.userStats.size });
    } catch (error) {
      loggingService.error('Failed to load historical data', { error: error.message });
    }
  }
    ...
    try {
      ...
      for (const period of periods) {
        try {
          ...
          loggingService.service('MonitoringService', 'metrics-loaded', { period, entries: this.metrics[period].size });
        } catch (fileError) {
          loggingService.service('MonitoringService', 'file-not-found', { file: `${period}_metrics.json` });
        }
      }
    } catch (error) {
      loggingService.error('Failed to load aggregated metrics', { error: error.message });
    }
  }
  ...
  startMetricsCollection() {
    ...
    loggingService.service('MonitoringService', 'realtime-collection-started');
  }
  ...
  startDataPersistence() {
    ...
    loggingService.service('MonitoringService', 'data-persistence-started');
  }
  ...
  async persistMetrics() {
    try {
      ...
      loggingService.service('MonitoringService', 'metrics-persisted');
    } catch (error) {
      loggingService.error('Failed to persist metrics', { error: error.message });
    }
  }
  ...
  async shutdown() {
    loggingService.service('MonitoringService', 'shutting-down');    
    ...
    loggingService.win('MonitoringService shutdown complete');
  }
}
