...
async function safeGetUserPreferences() {
    try {
        if (typeof getUserPreferences === 'function') {
            ...
            console.error('getUserPreferences function not found');
...
async function initCVBuilder() {
    ...
    try {
        ...
        console.log(`📝 Loading CV ${currentCV}:`, userPreferences.cvs[currentCV]?.personalInfo?.firstName);
...
function startAutoPreviewUpdate() {
    ...
    console.log('🔄 Auto preview update started (every 5 seconds)');
}
...
function updateInfoItem(containerId, contentId, value) {
    ...
    console.log(`Updating ${containerId}:`, value, 'Container:', !!container, 'Content:', !!content);
...
function debugDrivingLicense() {
    ...
    console.log('Input element:', !!input, input ? input.value : 'N/A');
    console.log('Container element:', !!container);
    console.log('Content element:', !!content);
    console.log('CV data:', userPreferences?.cvs?.[currentCV]?.personalInfo?.drivingLicense);
...
function updatePreview() {
    ...
    console.log('Driving License value:', cv.personalInfo.drivingLicense);
...
function initializeEventListeners() {
    ...
    if (cvSelector && !cvSelector.hasAttribute('data-listener')) {
        ...
        cvSelector.addEventListener('change', (e) => {
            console.log('CV Selector changed to:', e.target.value);
    ...
    if (newCVBtn && !newCVBtn.hasAttribute('data-listener')) {
        ...
        newCVBtn.addEventListener('click', () => {
            console.log('New CV button clicked');
            createNewCV();
        });
    }
    ...
    if (cvModeToggle && !cvModeToggle.hasAttribute('data-listener')) {
        ...
        cvModeToggle.addEventListener('click', () => {
            console.log('CV Mode toggle clicked');
            toggleCVMode();
        });
    }
    ...
    if (photoUpload && !photoUpload.hasAttribute('data-listener')) {
        ...
        photoUpload.addEventListener('change', (e) => {
            console.log('Photo upload changed');
            handlePhotoUpload(e);
        });
    }
    ...
    if (photoSizeSlider && !photoSizeSlider.hasAttribute('data-listener')) {
        ...
        photoSizeSlider.addEventListener('input', (e) => {
            console.log('Photo size changed to:', e.target.value);
...
function createRequiredElements() {
    ...
    console.log('✅ Required elements created for main.js detection');
}
...
if (typeof window !== 'undefined') {
    ...
    console.log('🚀 CV Builder functions loaded into window');
    
    createRequiredElements();
    
    setTimeout(() => {
        console.log('🎯 Auto-initializing CV Builder...');
