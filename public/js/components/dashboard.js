(function() {
    window.clientConfig&&window.clientConfig.smartLog('buffer','Dashboard component loading...');
    ...
    function cleanupOldDashboardData() {
        if (userData.dashboardData && userData.dashboardData.dailyOpportunities) {
            window.clientConfig&&window.clientConfig.smartLog('buffer','Cleaning up old dashboardData system');
    ...
    async function searchOpportunitiesFromSystemCache(jobTitle, today) {
        try {
            ...
            if (response.ok) {
                ...
                window.clientConfig&&window.clientConfig.smartLog('buffer',`Found ${allCacheResults.length} total offers in system cache`);
                
                const relevantJobs = findRelevantJobs(allCacheResults, jobTitle);
                
                window.clientConfig&&window.clientConfig.smartLog('buffer',`Found ${relevantJobs.length} relevant jobs for "${jobTitle}"`);
                window.clientConfig&&window.clientConfig.smartLog('buffer','Relevant jobs:', relevantJobs.map(job => job.title));
    ...
    window.safeSaveUserPreferences(userData);
                ...
                window.clientConfig&&window.clientConfig.smartLog('buffer','API not available, using fallback local search');
                fallbackToLocalSearch(jobTitle, today);
            }
        } catch (error) {
            window.clientConfig&&window.clientConfig.smartLog('fail','Error searching system cache:', error);
            fallbackToLocalSearch(jobTitle, today);
        }
    }
    ...
    function findRelevantJobs(allJobs, jobTitle) {
        ...
        window.clientConfig&&window.clientConfig.smartLog('buffer',`Searching for jobs matching "${titleLower}" (words: ${titleWords.join(', ')})`);
        ...
        const matchedJobs = allJobs.filter(job => {
            ...
            for (const searchText of searchTexts) {
                if (searchText.includes(titleLower)) {
                    window.clientConfig&&window.clientConfig.smartLog('buffer',`Direct match found: "${job.title}" contains "${titleLower}"`);
                    return true;
                }
                
                if (fuzzyMatch(searchText, titleLower, 90) >= 90) {
                    window.clientConfig&&window.clientConfig.smartLog('buffer',`Fuzzy match found: "${job.title}" matches "${titleLower}" with high similarity`);
                    return true;
                }
                ...
                for (const word of searchWords) {
                    if (word.length > 2) {
                        if (fuzzyMatch(word, titleLower, 90) >= 90) {
                            window.clientConfig&&window.clientConfig.smartLog('buffer',`Word match found: "${word}" in "${job.title}" matches "${titleLower}"`);
                            return true;
                        }
                        
                        for (const titleWord of titleWords) {
                            if (fuzzyMatch(word, titleWord, 90) >= 90) {
                                window.clientConfig&&window.clientConfig.smartLog('buffer',`Cross-word match found: "${word}" in "${job.title}" matches "${titleWord}"`);
                                return true;
                            }
                        }
                    }
        ...
        window.clientConfig&&window.clientConfig.smartLog('buffer',`Found ${matchedJobs.length} matching jobs from ${allJobs.length} total jobs`);
        return matchedJobs;
    }
    ...
    function fallbackToLocalSearch(jobTitle, today) {
        ...
        window.clientConfig&&window.clientConfig.smartLog('buffer','Fallback to local search with', uniqueResults.length, 'cached jobs');
    ...
    function displayProfileOpportunities() {
        ...
        window.clientConfig&&window.clientConfig.smartLog('buffer','Displaying opportunities:', opportunities.length, opportunities);
    ...
    function savePersonalReminder() {
        const reminderText = document.getElementById('personal-reminder-text')?.value || '';
        ...
        userData.personalReminder.lastUpdated = new Date().toISOString();
    ...
    function loadPersonalReminder() {
        const reminderInput = document.getElementById('personal-reminder-text');
        if (reminderInput && userData.personalReminder?.text) {
            reminderInput.value = userData.personalReminder.text;
        }
    }
    ...
    window.clientConfig&&window.clientConfig.smartLog('buffer','Dashboard component loaded successfully');
})();