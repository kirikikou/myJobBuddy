...
function initEmailSearch() {
    console.log('Initializing Email Search...');
...
async function updateEmailSearchLimits() {
    try {
        console.log('[EMAIL-LIMITS] Fetching email search limits...');
        const response = await fetch('/email-limits/status');
        const data = await response.json();
        
        console.log('[EMAIL-LIMITS] Response:', data);
        ...
        if (data.success) {
            ...
            if (emailStatusBar && emailRemainingText) {
                ...
                console.log(`[EMAIL-LIMITS] Plan: ${currentPlan}, Live search: ${canUseLive}`);
                ...
                if (isFreePlan) {
                    ...
                    console.log(`[EMAIL-LIMITS] Live: ${liveUsed}/${liveLimit}, Cache: ${cacheUsed}/${cacheLimit}`);
                ...
                console.error('[EMAIL-LIMITS] Status bar elements not found');
            }
        } else {
            console.error('[EMAIL-LIMITS] API error:', data.message);
        }
    } catch (error) {
        console.error('[EMAIL-LIMITS] Error updating email search limits:', error);
    }
}
...
function addDomain() {
    ...
    domainList.forEach(domainString => {
        ...
        if (domainRegex.test(domainOnly) && !domains.some(d => d.domain === domainOnly)) {
            ...
            console.warn(`Invalid domain skipped: ${domainString}`);
        }
    });
...
async function startDiscovery() {
    ...
    try {
        ...
        console.error('Error starting email exploration stream:', error);
...
async function startCacheOnlySearch() {
    ...
    try {
        ...
        console.error('Error during cache email search:', error);
...
function handleStreamEvent(data) {
    console.log('Email stream event:', data.phase, data);
    ...
    switch (data.phase) {
        ...
        case 'exploration-error':
            ...
            break;
        case 'complete':
            console.log('[COMPLETE] Received complete event with data:', data);
            handleComplete(data);
            break;
        case 'error':
            ...
            break;
        default:
            console.log('Unknown email stream phase:', data.phase);
    }
}
...
function handleComplete(data) {
    console.log('[HANDLE COMPLETE] Starting with data:', data);
    isSearching = false;
    allResults = data.results || [];
    totalProcessed = data.totalProcessed;
    
    console.log('[HANDLE COMPLETE] allResults:', allResults);
    console.log('[HANDLE COMPLETE] summary:', data.summary);
    ...
    console.log('[HANDLE COMPLETE] pendingResults created:', pendingResults);
    console.log('[HANDLE COMPLETE] countdownFinished:', countdownFinished, 'countdownInterval:', countdownInterval);
    
    console.log('[HANDLE COMPLETE] Forcing immediate display');
    displayPendingResults();
}

function displayPendingResults() {
    console.log('[DISPLAY PENDING] Starting with pendingResults:', pendingResults);
    
    if (!pendingResults) {
        console.log('[DISPLAY PENDING] No pending results - exiting');
        return;
    }
    ...
    console.log('[DISPLAY PENDING] Duration:', duration, 'Summary:', summary, 'Results count:', results.length);
    
    updateProgressText(`Complete! ${summary.totalEmailsFound || 0} emails found in ${duration}s`);
    
    allResults = results;
    
    console.log('[DISPLAY PENDING] Updated allResults:', allResults);
    ...
    console.log('[DISPLAY PENDING] Completed successfully');
}
...
function displayResults(data) {
    console.log('[DISPLAY RESULTS] Starting with data:', data);
    
    const summary = data.summary || {};
    const results = data.results || [];
    
    console.log('[DISPLAY RESULTS] Summary:', summary, 'Results:', results);
    ...
    console.log('[DISPLAY RESULTS] Updated stats UI');
    ...
    console.log('[DISPLAY RESULTS] Setting up filters and applying');
...
function applyFilters() {
    console.log('[APPLY FILTERS] Starting with allResults:', allResults);
    
    const domainFilter = document.getElementById('domainFilter')?.value.toLowerCase() || '';
    const emailFilter = document.getElementById('emailFilter')?.value.toLowerCase() || '';
    
    console.log('[APPLY FILTERS] Filters:', { domainFilter, emailFilter });
    
    filteredResults = [];
    
    for (const result of allResults) {
        if (!result.success || !result.emails) {
            console.log('[APPLY FILTERS] Skipping result:', result.domain, 'success:', result.success, 'emails:', result.emails);
            continue;
        }
    ...
    console.log('[APPLY FILTERS] filteredResults:', filteredResults);
    
    displayFilteredResults();
}

function displayFilteredResults() {
    console.log('[DISPLAY FILTERED] Starting with filteredResults:', filteredResults);
    
    let totalPages = 0;
    let totalContactPages = 0;
    
    const resultsHtml = filteredResults.map(result => {
        console.log('[DISPLAY FILTERED] Processing result:', result);
        ...
        console.log('[DISPLAY FILTERED] Processing emails for', result.domain, ':', result.emails);
    ...
    console.log('[DISPLAY FILTERED] Generated HTML length:', resultsHtml.length);
    ...
    console.log('[DISPLAY FILTERED] Setting innerHTML to:', finalHtml.substring(0, 200) + '...');
    
    resultsContainer.innerHTML = finalHtml;
    
    console.log('[DISPLAY FILTERED] Completed successfully');
}
