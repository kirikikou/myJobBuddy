(function() {
        ...
        return new Promise((resolve) => {
            function checkData() {
                if (window.userData && 
                    ...
                    console.log('UserData is ready:', window.userData);
                    isDataReady = true;
                    resolve();
                } else {
                    initRetryCount++;
                    if (initRetryCount < MAX_RETRY) {
                        console.log('Waiting for userData... attempt', initRetryCount);
                        setTimeout(checkData, 200);
                    } else {
                        console.error('Failed to load userData after max retries');
                        resolve();
                    }
                }
            }
    ...
    async function initJobSearch() {
        ...
        console.log('Starting job search initialization...');
        await waitForUserData();
        
        if (!isDataReady) {
            console.error('Could not initialize job search: userData not ready');
            return;
        }
        
        isInitialized = true;
        console.log('Job search initializing with userData:', userData);
        ...
        if (!userData.jobTitles) userData.jobTitles = [];
        if (!userData.careerPages) userData.careerPages = [];
        ...
        console.log('Job search initialization complete');
        cleanUserData();
    }
    ...
    async function performJobSearchWithNotifications() {
        ...
        window.clientConfig && window.clientConfig.smartLog('buffer', 'Starting EventSource connection to search-career-pages-stream');
        ...
        try {
            ...
            eventSource.onmessage = (event) => {
              try {
                ...
                window.clientConfig && window.clientConfig.smartLog('buffer', 'EventSource message received:', data.phase);
                ...
                window.clientConfig && window.clientConfig.smartLog('fail', 'Failed to parse EventSource data:', parseError.message);
                showToast('error', 'Error processing search response');
              }
            };
            ...
            eventSource.onerror = (error) => {
              ...
              stopCountdownTimer();
              loadingIndicator.style.display = 'none';
              showToast('error', 'Connection lost during search');
            };
            
        } catch (connectionError) {
            window.clientConfig && window.clientConfig.smartLog('fail', 'Failed to start EventSource connection:', connectionError.message);
            stopCountdownTimer();
            loadingIndicator.style.display = 'none';
            showToast('error', 'Failed to start search connection');
        }
    }
    ...
    async function performCacheOnlySearchWithNotifications() {
        ...
        try {
            ...
            console.error('Error during cache-only search:', error);
            loadingIndicator.style.display = 'none';
        ...
        const updateDisplay = () => {
            ...
            if (remainingSeconds < 0) {
                ...
                if (searchResultsReceived && pendingResults) {
                    console.log('[COUNTDOWN] Countdown finished - displaying pending results');
                    displayPendingResults();
                } else {
                    console.log('[COUNTDOWN] Countdown finished - waiting for results');
    ...
    function displayPendingResults() {
        if (!pendingResults) {
            console.log('[RESULTS] No pending results to display');
            return;
        }
        ...
        console.log('[RESULTS] Displaying results:', pendingResults);
    ...
    function showContextualMessage(searchType, contextData) {
        ...
        if (contextData.domainsNotified > 0) {
            message = `[BUFFER] ${contextData.domainsNotified} domains used intelligent buffer (instant results from ongoing scraping)`;
        } else if (searchType === 'cache_only') {
            message = `[CACHE] All results from cache (instant search)`;
        } else if (contextData.domainsCached > 0 && contextData.domainsScraped > 0) {
            message = `[MIXED] ${contextData.domainsCached} from cache + ${contextData.domainsScraped} freshly scraped`;
        }
    ...
    function exportJobResultsToHTML() {
        ...
        const jobRowsHTML = jobsData.map(job => `
        ...
        <script>
            ...
            function initializeStorage() {
                ...
                if (!storageAvailable) {
                    ...
                    storageInfo.innerHTML = 'Local storage not available. Data will only persist during this session.';
                }
    ...
    function setupKeyboardNavigation() {
        document.removeEventListener('keydown', globalDeleteHandler);
        document.addEventListener('keydown', globalDeleteHandler);
        ...
        if (activeElement && (activeElement === jobTitleInput || activeElement === careerUrlInput)) {
            ...
            if (cursorPosition === 0 && inputValue.length === 0) {
                ...
                if (activeElement === jobTitleInput) {
                    container = document.getElementById('jobTitlesContainer');
                    if (window.clientConfig) {
                        window.clientConfig.smartLog('win', 'Delete key pressed in job title input - targeting jobTitlesContainer');
                    }
                } else if (activeElement === careerUrlInput) {
                    container = document.getElementById('careerUrlsContainer');
                    if (window.clientConfig) {
                        window.clientConfig.smartLog('win', 'Delete key pressed in career URL input - targeting careerUrlsContainer');
                    }
                }
                ...
                if (container) {
                    ...
                    if (window.clientConfig) {
                        window.clientConfig.smartLog('win', `Found ${tags.length} tags in container`);
                    }
                    if (tags.length > 0) {
                        const lastTag = tags[tags.length - 1];
                        if (window.clientConfig) {
                            window.clientConfig.smartLog('win', `Deleting last tag: ${lastTag.textContent}`);
                        }
                        deleteTag(lastTag);
                    }
                } else {
                    if (window.clientConfig) {
                        window.clientConfig.smartLog('fail', 'Container not found for delete operation');
                    }
                }
            }
        } else {
            if (window.clientConfig && activeElement && (e.key === 'Delete' || e.key === 'Backspace')) {
                window.clientConfig.smartLog('win', `Delete key pressed on element: ${activeElement.id || activeElement.tagName || 'unknown'}`);
            }
        }
    }
    ...
    function deleteTag(tag) {
        ...
        if (tag.classList.contains('favorite-tag')) {
            ...
            if (closeBtn) {
                tagText = tag.textContent.replace(closeBtn.textContent, '').trim();
            } else {
                tagText = tag.textContent.replace(/x$/, '').trim();
            }
        }
        
        if (window.clientConfig) {
            window.clientConfig.smartLog('win', `Extracted tag text: "${tagText}" from tag content: "${tag.textContent}"`);
        }
        
        if (container.id === 'jobTitlesContainer') {
            const originalLength = userData.jobTitles.length;
            userData.jobTitles = userData.jobTitles.filter(t => t !== tagText);
            
            if (userData.jobTitles.length !== originalLength) {
                tag.remove();
                
                if (window.safeSaveUserPreferences) {
                    window.safeSaveUserPreferences(userData).then(result => {
                        if (window.clientConfig) {
                            window.clientConfig.smartLog('win', `Job title deleted and saved: ${tagText}`);
                        }
                    }).catch(error => {
                        if (window.clientConfig) {
                            window.clientConfig.smartLog('fail', `Failed to save job title deletion: ${error.message}`);
                        }
                    });
                } else {
                    saveUserData();
                ...
                if (window.clientConfig) {
                    window.clientConfig.smartLog('win', `Job title deleted: ${tagText}`);
                }
            }
            ...
            if (tag.classList.contains('favorite-tag')) {
                    ...
                    showElegantConfirm(
                        ...
                        () => {
                            ...
                            if (window.safeSaveUserPreferences) {
                                window.safeSaveUserPreferences(userData).then(result => {
                                    if (window.clientConfig) {
                                        window.clientConfig.smartLog('win', `Favorite company deleted and saved: ${company.name}`);
                                    }
                                });
                            } else {
                                saveUserData();
                    ...
                    showElegantConfirm(
                        ...
                        () => {
                            ...
                            if (window.safeSaveUserPreferences) {
                                window.safeSaveUserPreferences(userData).then(result => {
                                    if (window.clientConfig) {
                                        window.clientConfig.smartLog('win', `Selection ${selectionType} company deleted and saved: ${company.name}`);
                                    }
                                });
                            } else {
                                saveUserData();
                ...
                const originalLength = userData.careerPageLists[activeList].length;
                userData.careerPageLists[activeList] = userData.careerPageLists[activeList].filter(p => p.url !== tagText);
                
                if (userData.careerPageLists[activeList].length !== originalLength) {
                    tag.remove();
                    
                    if (window.safeSaveUserPreferences) {
                        window.safeSaveUserPreferences(userData).then(result => {
                            if (window.clientConfig) {
                                window.clientConfig.smartLog('win', `Career URL deleted and saved from ${activeList}: ${tagText}`);
                            }
                        }).catch(error => {
                            if (window.clientConfig) {
                                window.clientConfig.smartLog('fail', `Failed to save career URL deletion: ${error.message}`);
                            }
                        });
                    } else {
                        saveUserData();
                    ...
                    if (window.clientConfig) {
                        window.clientConfig.smartLog('win', `Career URL deleted from ${activeList}: ${tagText}`);
                    }
                }
            }
        }
    ...
    function restoreSearchState() {
        ...
        console.log('Restoring selected site:', savedSite);
        ...
        if (userData.jobSearchData.lastSearchResults?.length > 0) {
            console.log('Restoring search results:', userData.jobSearchData.lastSearchResults.length, 'results');
    ...
    function setupEventListeners() {
        ...
        if (clearJobTitles) {
            clearJobTitles.addEventListener('click', function(e) {
                e.preventDefault();
                e.stopPropagation();
                console.log('Clear job titles clicked');
                if (userData.jobTitles.length === 0) {
        ...
        if (clearAllUrls) {
            clearAllUrls.addEventListener('click', function(e) {
                e.preventDefault();
                e.stopPropagation();
                console.log('Clear all URLs clicked');
                const activeList = getCurrentActiveList();
        ...
        if (searchButton) {
            ...
            searchButton.removeEventListener('click', performJobSearchWithNotifications);
            
            searchButton.addEventListener('click', function(e) {
                e.preventDefault();
                e.stopPropagation();
                console.log('SEARCH BUTTON CLICKED - Starting job search process');
                performJobSearchWithNotifications();
            });
        ...
        if (searchCacheButton) {
            searchCacheButton.removeEventListener('click', performCacheOnlySearchWithNotifications);
            searchCacheButton.addEventListener('click', function(e) {
                e.preventDefault();
                e.stopPropagation();
                console.log('Cache search button clicked');
                performCacheOnlySearchWithNotifications();
            });
        }
    }
        ...
        if (input.includes(',')) {
            ...
            urls.forEach(url => {
                try {
                    ...
                    if (!userData.careerPageLists[activeList].some(page => page.url === validatedUrl)) {
    ...
    function showElegantConfirm(title, message, onConfirm, confirmText = 'Delete', confirmColor = 'danger') {
        console.log('showElegantConfirm called:', title);
    ...
    function showLimitExceededModal(limitType, needed, available, currentPlan) {
        console.log('showLimitExceededModal called:', limitType, needed, available, currentPlan);
    ...
    function populateJobTitles() {
        ...
        if (!container || !input || !userData?.jobTitles) return;
        ...
        userData.jobTitles.forEach(title => {
            ...
            tag.className = 'tag';
            tag.tabIndex = 0;
            tag.innerHTML = `${title}<span class="tag-close">x</span>`;
    ...
    function cleanUserData() {
        if (userData.jobSearchData) {
            ...
            if (userData.jobSearchData.lastSearchResults) {
                const cleanResults = userData.jobSearchData.lastSearchResults.map(result => ({
                    title: result.title,
                    url: result.url,
    ...
    window.safeSaveUserPreferences(userData);
            ...
            console.log('User data cleaned - removed historical results');
        }
    }
    ...
    function populateCareerPages() {
        ...
        if (!container || !input || !userData) return;
        ...
        currentUrls.forEach(page => {
            ...
            tag.className = 'tag';
            tag.tabIndex = 0;
            tag.innerHTML = `${page.url}<span class="tag-close">x</span>`;
    ...
    function addApplicationDirectly(jobTitle, companyName, location, website, career) {
        ...
        if (!userData.companies) userData.companies = {};
    ...
    async function updateSearchLimits() {
        try {
            console.log('[LIMITS] Fetching user limits...');
            const response = await fetch('/plan/limits');
            const data = await response.json();
            ...
            if (data.success) {
                ...
                if (statusBar && remainingText) {
                    ...
                    console.log(`[LIMITS] Plan: ${currentPlan}, Live search: ${canPerformLiveSearch}`);
                    ...
                    if (isFreePlan) {
                        ...
                        console.log(`[LIMITS] Usage: ${used}/${limit} (${remaining} remaining)`);
                    ...
                    console.error('[LIMITS] Status bar elements not found');
                }
            } else {
                console.error('[LIMITS] API error:', data.message);
            }
        } catch (error) {
            console.error('[LIMITS] Error updating search limits:', error);
        }
    }
    ...
    window.getComponentData = function() {
        return {
            ...
            careerPages: userData.careerPages || [],
            locations: userData.locations || [],
            companies: userData.companies || {},
            jobSearchData: userData.jobSearchData || {}
        };
    };
    ...
    window.performJobSearchWithNotifications = performJobSearchWithNotifications;
    window.performCacheOnlySearchWithNotifications = performCacheOnlySearchWithNotifications;
    ...
    function cleanupJobSearchModule() {
        ...
        if (searchButton) {
            ...
            searchButton.parentNode.replaceChild(clonedButton, searchButton);
            clonedButton.addEventListener('click', function(e) {
                e.preventDefault();
                e.stopPropagation();
                window.clientConfig && window.clientConfig.smartLog('buffer', 'Clean search button clicked - starting job search process');
                performJobSearchWithNotifications();
            });
        }
    }
    ...
    window.clientConfig&&window.clientConfig.smartLog('win', 'Job Search module fully loaded and exposed');
