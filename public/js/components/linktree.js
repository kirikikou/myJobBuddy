(function() {
    ...
    function initLinktree() {
        if (!window.userData) {
            window.clientConfig&&window.clientConfig.smartLog('buffer','userData not ready, retrying...');
    ...
    function initializeLinktreeData() {
        if (!userData.linktrees || Object.keys(userData.linktrees).length === 0) {
            ...
            window.clientConfig && window.clientConfig.smartLog('buffer', 'Linktree: initialized default structure (no auto-save)');
        }
    }
    
    async function saveLinktreeData() {
        ...
        try {
            if (window.unifiedPreferencesService) {
                ...
                if (result) {
                    ...
                    showLocalizedToast('error', 'linktree.saveError');
                }
            } else if (window.safeSaveUserPreferences) {
                const result = await window.safeSaveUserPreferences(userData);
                if (result && result.success) {
                    const updatedData = await window.loadUserData();
                    if (updatedData) {
                        window.userData = updatedData;
                        userData = updatedData;
                    }
            ...
            window.clientConfig && window.clientConfig.smartLog('fail', `Linktree save error: ${error.message}`);
            showLocalizedToast('error', 'linktree.saveError');
        }
    }
    
    async function performClearLinktree() {
        if (!currentEditingId) return;
        ...
        try {
            if (window.unifiedPreferencesService) {
                ...
                if (result) {
                    ...
                    showLocalizedToast('error', 'linktree.clearError');
                }
            } else if (window.safeSaveUserPreferences) {
                const result = await window.safeSaveUserPreferences(userData);
                if (result && result.success) {
            ...
            window.clientConfig && window.clientConfig.smartLog('fail', `Linktree clear error: ${error.message}`);
            showLocalizedToast('error', 'linktree.clearError');
        }
    }
