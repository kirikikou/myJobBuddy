(function(){
    ...
    window.safeSaveUserPreferences = async (p) => {
        ...
        if (typeof p !== 'object') {
          if (window.clientConfig) {
            window.clientConfig.smartLog('fail', `safeSaveUserPreferences: invalid payload type ${typeof p}, skipping save`);
          }
...
async function verifyAuthentication() {
    try {
        ...
        window.clientConfig.smartLog('fail', `Auth verification failed: ${error.message}`);
        window.location.href = '/login';
        return false;
    }
}
    ...
    return new Promise((resolve) => {
        ...
        function checkDataComplete() {
            ...
            if (window.userData && 
                ...
                window.clientConfig.smartLog('buffer', 'All data structures confirmed loaded');
            ...
            if (attempts < maxAttempts) {
                ...
                setTimeout(checkDataComplete, 200);
            } else {
                window.clientConfig.smartLog('fail', 'Max attempts reached, proceeding with available data');
                resolve();
            }
        }
...
function showToast(type, message, options = {}) {
    ...
    if (type === 'success') {
        ...
        toastTitle.textContent = options.title || getTranslatedMessage('common.success') || 'Succès';
...
function showContextualMessage(type, data) {
    ...
    if (type === 'queue_optimization') {
        ...
        showToast('shared', 
            getTranslatedMessage('messages.sharedCache', {totalResults, domainsNotified, efficiency}) || `${totalResults} résultats trouvés`,
            { 
