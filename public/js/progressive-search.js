class ProgressiveSearchManager {
    ...
    async startProgressiveSearch(companies, jobTitles, locations = [], options = {}) {
      if (this.isSearching) {
        window.clientConfig&&window.clientConfig.smartLog('buffer','Search already in progress');
        return false;
      }
      ...
      try {
        ...
        window.clientConfig&&window.clientConfig.smartLog('fail','Progressive search error:', error);
        this.isSearching = false;
    ...
    async startCacheOnlySearch(companies, jobTitles, locations = [], options = {}) {
      try {
        ...
        window.clientConfig&&window.clientConfig.smartLog('fail','Cache-only search error:', error);
        this.trigger('searchError', { error: error.message });
        return false;
      }
    }
    ...
    setupEventStream() {
      if (!this.currentSearchId) {
        window.clientConfig&&window.clientConfig.smartLog('fail','No search ID available for event stream');
        return;
      }
      ...
      this.eventSource.onopen = () => {
        window.clientConfig&&window.clientConfig.smartLog('buffer',`Event stream connected for search ${this.currentSearchId}`);
      };
  
      this.eventSource.addEventListener('connected', (event) => {
        const data = JSON.parse(event.data);
        window.clientConfig&&window.clientConfig.smartLog('buffer','Event stream connected:', data);
      });
      ...
      this.eventSource.onerror = (error) => {
        window.clientConfig&&window.clientConfig.smartLog('fail','Event stream error:', error);
    ...
    async cancelSearch() {
      ...
      try {
        ...
        window.clientConfig&&window.clientConfig.smartLog('fail','Cancel search error:', error);
        return false;
      }
    }
      ...
      if (this.callbacks[event]) {
        this.callbacks[event].forEach(callback => {
          try {
            ...
            window.clientConfig&&window.clientConfig.smartLog('fail',`Error in event callback for ${event}:`, error);
          }
        });
      }
    }
  ...
  function initializeProgressiveSearch(containerId, options = {}) {
    ...
    if (!container) {
      window.clientConfig&&window.clientConfig.smartLog('fail',`Container ${containerId} not found`);
      return null;
    }
