  ...
  class UserPreferencesManager {
    ...
    saveUserPreferences(preferences, force = false) {
      ...
      if (!force && this.lastSentHash === prefsHash) {
        console.log('[UserPrefs] No changes detected, skipping save');
      ...
      return this.debounceManager.deduplicateRequest(saveKey, () => {
        return new Promise((resolve) => {
          this.debounceManager.debounce('save-user-prefs', async () => {
            try {
              console.log('[UserPrefs] Saving preferences...');
              ...
              if (response.status === 204) {
                console.log('[UserPrefs] No changes on server side');
              ...
              if (response.status === 429) {
                ...
                console.warn('[UserPrefs] Rate limited:', errorData.retryAfter);
              ...
              if (result.success) {
                ...
                console.log('[UserPrefs] Preferences saved successfully');
                showToast('success', 'Preferences saved');
              } else {
                console.error('[UserPrefs] Save failed:', result.message);
              ...
              console.error('[UserPrefs] Save error:', error);
    ...
    async getPlanLimits(forceRefresh = false) {
      ...
      if (!forceRefresh && 
        ...
        console.log('[UserPrefs] Plan limits from client cache');
      ...
      return this.debounceManager.deduplicateRequest(limitsKey, async () => {
        try {
          console.log('[UserPrefs] Fetching plan limits...');
          ...
          console.log('[UserPrefs] Plan limits cached for 30s');
          return result;
          
        } catch (error) {
          console.error('[UserPrefs] Plan limits fetch error:', error);
          // Return cached data if available, even if stale
          if (this.planLimitsCache) {
            console.log('[UserPrefs] Returning stale plan limits cache');
            return this.planLimitsCache;
          }
          throw error;
        }
    ...
    async loadUserPreferences() {
      try {
        ...
        if (result.success) {
          ...
          console.log('[UserPrefs] Preferences loaded');
        }
      } catch (error) {
        console.error('[UserPrefs] Load error:', error);
      }
    }
