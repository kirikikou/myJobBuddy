...
if (config.DEBUG) {
  ...
  config.smartLog('buffer', 'queue-gate:router-mounted:debug');
}
    ...
    const additionalJS = `
      ...
      function searchDomains(query) {
        fetch('/debug/api/search-domains?q=' + encodeURIComponent(query) + '&limit=20')
          ...
          .catch(error => {
            config.smartLog('fail','Erreur de recherche:', error);
          });
      }
...
router.get('/user', async (req, res) => {
  try {
    ...
    config.smartLog('buffer',`[DEBUG] Debugging user: email=${email}, userId=${userId}`);
    ...
    config.smartLog('fail','User debug error:', error);
    res.status(500).json({
      success: false,
      message: error.message,
      stack: error.stack
