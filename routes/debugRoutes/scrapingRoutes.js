...
router.get('/test', async (req, res) => {
  ...
  try {
    ...
    console.log(`[ScrapingTest] Testing scraping for ${url} (useCache: ${useCache}, timeout: ${timeout}s)`);
    ...
    if (useCache) {
      ...
      if (result) {
        console.log(`[ScrapingTest] Cache hit for ${url}`);
        source = 'cache';
      } else {
        console.log(`[ScrapingTest] Cache miss for ${url}`);
      }
    }
    
    if (!result) {
      console.log(`[ScrapingTest] Starting fresh scrape for ${url}`);
      ...
      try {
        ...
        if (result && useCache) {
          ...
          console.log(`[ScrapingTest] Result cached for ${url}`);
        }
      } catch (scrapingError) {
        console.error(`[ScrapingTest] Scraping failed for ${url}:`, scrapingError.message);
    ...
    console.error(`[ScrapingTest] Error:`, error.message);
    res.status(500).json({
      success: false,
