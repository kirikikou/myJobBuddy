...
router.get('/test', async (req, res) => {
  ...
  try {
    ...
    loggingService.steps('scraping-test-started', 'testing', { url, useCache, timeoutSeconds: timeout });    
    ...
    if (useCache) {
      ...
      if (result) {
        loggingService.cache('hit', url);
        source = 'cache';
      } else {
        loggingService.cache('miss', url);
      }
    }
    
    if (!result) {
      loggingService.scraper('starting-fresh-scrape', url);      
      ...
      try {
        ...
        if (result && useCache) {
          ...
          loggingService.cache('set', url);
        }
      } catch (scrapingError) {
        loggingService.fail('Scraping failed', { url, error: scrapingError.message });        
    ...
    loggingService.error('ScrapingTest error', { error: error.message });
      res.status(500).json({
      success: false,
