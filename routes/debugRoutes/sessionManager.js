...
const loadAllSessionsFromFiles = async () => {
  try {
    ...
    console.error('Failed to load sessions from files:', error.message);
    return new Map();
  }
};
...
const organizeLogsByDomain = (logs) => {
  ...
  Object.keys(domainLogs).forEach(domain => {
    domainLogs[domain].logs.sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp));
  });
...
const saveLogToFile = async (logEntry) => {
  try {
    ...
    console.error('Failed to save console log:', error.message);
  }
};
            ...
            return `
              <div class="session-card" style="background: #f8f9fa; border: 1px solid #dee2e6; border-radius: 4px; padding: 15px; margin: 10px 0;">
                ...
                <div style="margin-top: 10px;">
                  ...
                  <a href="/debug/console/sessions/${session.id}/domains" class="button small secondary">By Domain</a>
                </div>
              </div>
            `;
          }).join('')
...
router.get('/sessions/:sessionId/domains', async (req, res) => {
  try {
    ...
    const domainLogs = organizeLogsByDomain(session.logs);
    const domains = Object.keys(domainLogs).sort();
    ...
    if (acceptHeader.includes('text/html')) {
      ...
      const domainsList = domains.map(domain => {
        const domainData = domainLogs[domain];
        const isSelected = selectedDomain === domain;
        const successRate = domainData.stats.total > 0 ? Math.round((domainData.stats.successes / domainData.stats.total) * 100) : 0;
        ...
        return `
          <div class="domain-card" style="background: ${isSelected ? '#e3f2fd' : '#f8f9fa'}; border: 2px solid ${isSelected ? '#2196f3' : '#dee2e6'}; border-radius: 8px; padding: 15px; margin: 10px 0; cursor: pointer;" onclick="selectDomain('${domain}')">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
              <h4 style="margin: 0; color: #333; font-size: 16px;">${domain === 'general' ? '🌐 General Logs' : domain === 'invalid-urls' ? '⚠️ Invalid URLs' : '🌍 ' + domain}</h4>
      ...
      const selectedDomainLogs = selectedDomain && domainLogs[selectedDomain] 
        ? domainLogs[selectedDomain].logs.map(log => `
      ...
      const content = `
        ...
        <div style="display: grid; grid-template-columns: 1fr 2fr; gap: 20px;">
          ...
          <div class="section">
            <h2>Domain Logs</h2>
            ${selectedDomain ? `
              <h3>📍 ${selectedDomain} (${domainLogs[selectedDomain].stats.total} logs)</h3>
  ...
  try {
    ...
    if (acceptHeader.includes('text/html')) {
      ...
      const content = `
        ...
        <div class="actions">
          <a href="/debug/console/sessions/${sessionId}" class="button secondary">Back to Session</a>
          <a href="/debug/console/sessions/${sessionId}/domains" class="button secondary">View by Domain</a>
          <a href="/debug/console/sessions" class="button secondary">All Sessions</a>
        </div>
