...
router.get('/explore-domains-stream', 
  ...
  async (req, res) => {
    ...
    loggingService.service('EmailStream', 'user-request', { userId, domainsCount: domains.length, maxDepth });
    loggingService.service('EmailStream', 'search-requirements', { needsLiveSearch, needsCacheSearch });
    ...
    const sendEvent = (data) => {
      try {
        ...
        loggingService.error('Error sending SSE event:',{ error: error });
      }
    };
    ...
    try {
      ...
      for (const domain of domains) {
        try {
          ...
          loggingService.error('EmailStream cache check error', { domain, error: error.message });
          staleOrMissingDomains.push(domain);
        }
      }
      ...
      if (staleOrMissingDomains.length > 0) {
        ...
        const explorationPromises = staleOrMissingDomains.map(async (domain) => {
          try {
            ...
            loggingService.fail('EmailStream exploration failed', { domain, error: error.message });            
            results.push({
              domain: new URL(domain).hostname,
              success: false,
              error: error.message,
      ...
      loggingService.win('EmailStream completed', { userId, totalEmails, processingTimeMs: summary.processingTimeMs });
      loggingService.service('EmailStream', 'credits-used', { liveCredits: actualLiveUsed, cacheCredits: actualCacheUsed });

    } catch (error) {
loggingService.error('EmailStream fatal error', { userId, error: error.message });      
      sendEvent({
        phase: 'error',
      ...
      try {
        ...
        loggingService.error('Error closing SSE connection:',{ error: resError });
      }
    }
  }
);
      ...
      for (const domain of domains) {
        try {
          ...
          if (cached && !isStale(cached)) {
            loggingService.cache('hit', domain);
            results.push({
            ...
            loggingService.service('EmailSearch', 'exploring-domain', { domain });
          ...
          loggingService.fail('EmailSearch exploration failed', { domain, error: error.message });
          results.push({
            domain: new URL(domain).hostname,
            success: false,
            error: error.message,
...
router.get('/email-history', isAuthenticated, async (req, res) => {
  try {
    ...
    loggingService.service('emailSearchRoutes','log',{ message: 'Looking for cache in:', details: cacheDir });
    ...
    loggingService.error('Email history error:',{ error: error });
    res.status(500).json({
      success: false,
      message: error.message
    });
    ...
    try {
      ...
      for (const domain of domains) {
        try {
          ...
          loggingService.error('EmailCache load error', { domain, error: error.message });
        }
      }
