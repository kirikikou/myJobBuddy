...
router.get('/explore-domains-stream', 
  ...
  async (req, res) => {
    ...
    console.log(`[Email Stream] User ${userId} requesting exploration of ${domains.length} domains with depth ${maxDepth}`);
    console.log(`[Email Stream] Needs: ${needsLiveSearch} live, ${needsCacheSearch} cache searches`);
    ...
    try {
      ...
      for (const domain of domains) {
        try {
          ...
          console.error(`[Email Stream] Cache check error for ${domain}:`, error);
          staleOrMissingDomains.push(domain);
        }
      }
      ...
      if (staleOrMissingDomains.length > 0) {
        ...
        const explorationPromises = staleOrMissingDomains.map(async (domain) => {
          try {
            ...
            console.error(`[Email Stream] Error exploring ${domain}:`, error);
      ...
      console.log(`[Email Stream] Completed for user ${userId}: ${totalEmails} emails, ${summary.processingTimeMs}ms`);
      console.log(`[Email Stream] Credits used - Live: ${actualLiveUsed}, Cache: ${actualCacheUsed}`);

    } catch (error) {
      console.error(`[Email Stream] Fatal error for user ${userId}:`, error);
      ...
      try {
        ...
        console.error('Error closing SSE connection:', resError);
      }
    }
  }
);
      ...
      for (const domain of domains) {
        try {
          ...
          if (cached && !isStale(cached)) {
            console.log(`[Email Search] Using cached results for ${domain}`);
            results.push({
            ...
            console.log(`[Email Search] Exploring ${domain}`);
          ...
          console.error(`[Email Search] Error exploring ${domain}:`, error);
          results.push({
            domain: new URL(domain).hostname,
            success: false,
            error: error.message,
...
router.get('/email-history', isAuthenticated, async (req, res) => {
  try {
    ...
    console.log('Looking for cache in:', cacheDir);
    ...
    try {
      ...
      for (const domain of domains) {
        try {
          ...
          console.error(`[Email Cache Search] Error loading cache for ${domain}:`, error);
        }
      }
