...
router.get('/limits', isAuthenticated, async (req, res) => {
  try {
    ...
    if (cachedResult) {
      logger.info('Plan limits served from cache', { cache: 'hit' });
      return res.json(cachedResult);
    }
    
    logger.info('Plan limits cache miss, fetching fresh data', { cache: 'miss' });
    ...
    logger.info('Plan limits cached for 30s', { plan: userPlan, cache: 'stored' });
    ...
    const apiContext = config.createApiContext(req);
    const logger = config.getContextualLogger(req.sessionID, apiContext);
    logger.error('Error in /plan/limits', { error: error.message });
...
router.post('/upgrade', isAuthenticated, async (req, res) => {
  try {
    ...
    const apiContext = config.createApiContext(req);
    const logger = config.getContextualLogger(req.sessionID, apiContext);
    logger.error('Upgrade error', { error: error.message });
