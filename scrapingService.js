...
async function exportTimingReport(sessionId, results) {
  ...
  config.smartLog('timing', `ðŸ” exportTimingReport called: sessionId=${sessionId}, timingLogs=${timingLogs.length}, results=${results.length}`);
  
  if (timingLogs.length === 0) {
    config.smartLog('fail', `âš ï¸ NO TIMING LOGS TO EXPORT - timingLogs array is empty!`);
    return null;
  }
  ...
  try {
    ...
    config.smartLog('parallel', `âœ… TIMING REPORT EXPORTED: ${filePath}`);
    config.smartLog('timing', `ðŸ“Š Total logs: ${timingLogs.length}, Start times: ${startTimes.length}`);
    
    timingLogs.length = 0;
    
    return filePath;
  } catch (error) {
    config.smartLog('fail', `âŒ Failed to export timing report: ${error.message}`);
...
async function ensureDebugDirectoryExists() {
  try {
    ...
    config.smartLog('timing', `âœ… Debug directory verified: ${config.DEBUG_DIR}`);
    return true;
  } catch (error) {
    config.smartLog('fail', `âŒ Debug directory error: ${error.message}`);
    return false;
  }
}
...
async function processBatch(batch, sessionId, options, logger, batchIndex, totalBatches, onPartialResult = null) {
  ...
  logTiming(`ðŸš€ BATCH ${batchIndex + 1} STARTING - URLs: ${batch.length}`);
  
  const batchPromises = batch.map(async (url, index) => {
    const urlStartTime = Date.now();
    const globalIndex = batchIndex * batch.length + index + 1;
    
    logTiming(`ðŸš€ URL${globalIndex} START`, url, globalIndex);
    ...
    try {
      ...
      if (options.useCache !== false) {
        ...
        if (result) {
          ...
          logTiming(`âœ… URL${globalIndex} END (CACHE) - Duration: ${duration}ms`, url, globalIndex);
        } else {
      ...
      if (!result) {
        ...
        logTiming(`ðŸ”¥ URL${globalIndex} SCRAPING START`, url, globalIndex);
        ...
        logTiming(`ðŸ URL${globalIndex} SCRAPING END - Duration: ${scrapingDuration}ms`, url, globalIndex);
      ...
      if (result && result.data) {
        ...
        logTiming(`âœ… URL${globalIndex} TOTAL END - Duration: ${duration}ms`, url, globalIndex);
        ...
        const duration = Date.now() - urlStartTime;
        logTiming(`âŒ URL${globalIndex} FAILED END - Duration: ${duration}ms`, url, globalIndex);
      ...
      const duration = Date.now() - urlStartTime;
      logTiming(`ðŸ’¥ URL${globalIndex} ERROR END - Duration: ${duration}ms - Error: ${error.message}`, url, globalIndex);
  ...
  logTiming(`â³ BATCH ${batchIndex + 1} - All ${batch.length} promises created, waiting for Promise.allSettled...`);
  
  const batchResults = await Promise.allSettled(batchPromises);
  
  const batchDuration = Date.now() - batchStartTime;
  logTiming(`ðŸ BATCH ${batchIndex + 1} COMPLETE - Duration: ${batchDuration}ms`);
...
async function scrapeCareerPage(url, options = {}, userId = 'anonymous', userEmail = null) {
  const sessionId = await sessionManager.startSession(userId, userEmail, `Single URL: ${url}`, [url]);
  ...
  try {
    ...
    if (options.skipCoordination) {
      config.smartLog('steps', `Direct scraping without coordination for ${url}`);
...
async function exportParallelReport(sessionId, performanceReport, results) {
  ...
  try {
    ...
    config.smartLog('parallel', `ðŸ“„ REPORT EXPORTED: ${filePath}`);
    if (performanceReport?.speedupRatio) {
      config.smartLog('timing', `ðŸŽ¯ PARALLEL ANALYSIS: ${detailedReport.summary.speedupRatio.toFixed(1)}x speedup, ${detailedReport.summary.parallelEfficiency}% efficiency`);
    }
...
async function createParallelDiagnosticReport(urls, results, sessionId) {
  ...
  try {
    ...
    config.smartLog('parallel', `ðŸ” DIAGNOSTIC EXPORTED: ${filePath}`);
    return filePath;
  } catch (error) {
