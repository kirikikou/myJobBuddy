...
// Créer le dossier de débogage si nécessaire
const ensureDebugDir = async () => {
  if (config.DEBUG) {
    try {
      await fs.mkdir(config.DEBUG_DIR, { recursive: true });
    } catch (err) {
      config.smartLog('fail','Erreur lors de la création du dossier de débogage:', err);
    }
  }
};
...
// Fonction pour détecter si une date est plus récente qu'un mois
const isWithinOneMonth = (dateText) => {
  ...
  try {
    ...
    config.smartLog('buffer',`Erreur d'analyse de date: ${dateText}`, e);
  }
  
  return true;
};

// Fonction pour vérifier les correspondances de mots-clés
