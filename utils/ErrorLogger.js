...
class ErrorLogger {
  ...
  async init() {
    try {
      ...
      console.error('Failed to initialize ErrorLogger:', error.message);
    }
  }
  ...
  async persistError(errorData) {
    try {
      ...
      console.error('Failed to persist error:', error.message);
    }
  }
  ...
  async updateErrorStats(errorData) {
    try {
      ...
      console.log(`⚠️ [ErrorLogger] Stats update failed silently: ${error.message}`);
    }
  }
  ...
  triggerAlert(errorData) {
    console.log(`🚨 ERROR ALERT - ${errorData.severity.toUpperCase()} SEVERITY`);
    console.log(`Domain: ${errorData.domain}`);
    console.log(`Type: ${errorData.errorType}`);
    console.log(`Message: ${errorData.message}`);
    console.log(`Step: ${errorData.step}`);
    console.log(`Time: ${errorData.timestamp}`);
    
    if (errorData.userId) {
      console.log(`User: ${errorData.userId} (${errorData.userEmail || 'No email'})`);
    }
  }
  ...
  async detectEmptyDomains() {
    try {
      ...
      console.error('Failed to detect empty domains:', error.message);
      return [];
    }
  }
