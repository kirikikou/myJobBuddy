...
class ErrorLogger {
  ...
  async init() {
    try {
      ...
      loggingService.error('Failed to initialize ErrorLogger:',{ error: error.message });
    }
  }
  ...
  async persistError(errorData) {
    try {
      ...
      loggingService.error('Failed to persist error:',{ error: error.message });
    }
  }
  ...
  async updateErrorStats(errorData) {
    try {
      ...
      loggingService.error('ErrorLogger stats update failed silently', { error: error.message });
    }
  }
  ...
  triggerAlert(errorData) {
    loggingService.alert('ERROR ALERT', {
      ...
      userId: errorData.userId,
      userEmail: errorData.userEmail || 'No email'
    });
  }
  ...
  async detectEmptyDomains() {
    try {
      ...
      loggingService.error('Failed to detect empty domains:',{ error: error.message });
      return [];
    }
  }
