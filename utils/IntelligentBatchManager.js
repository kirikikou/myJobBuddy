...
class IntelligentBatchManager {
  ...
  async initialize() {
    ...
    console.log('🧠 IntelligentBatchManager initialized');
  }
  ...
  async processBatch(domains, options = {}) {
    ...
    console.log(`🧠 Processing batch ${batchId} with ${domains.length} domains`);
    ...
    console.log(`📊 Batch analysis: ${batchConfig.batches} batches, ${batchConfig.parallel} parallel, ${Math.round(estimatedWaitTime)}min wait`);
    
    if (!forceServer && estimatedWaitTime > parallelizationConfig.MAX_WAIT_TIME_MINUTES) {
      console.log(`⚡ Batch ${batchId} delegated to AWS (wait time: ${Math.round(estimatedWaitTime)}min > ${parallelizationConfig.MAX_WAIT_TIME_MINUTES}min)`);
  ...
  async processOnServer(domains, batchConfig, batchId, options = {}) {
    console.log(`🖥️ Processing ${batchId} on server: ${batchConfig.batches} batches, ${batchConfig.parallel} parallel`);
    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      if (i < batches.length - 1) {
        ...
        if (delayMs > 0) {
          console.log(`⏳ Inter-batch delay: ${delayMs}ms`);
    ...
    console.log(`✅ Batch ${batchId} completed in ${Math.round(totalDuration / 1000)}s`);
  ...
  async processBatchParallel(batch, options = {}) {
    ...
    const promises = batch.map(async (domain) => {
      try {
        ...
        console.error(`❌ Batch processing failed for ${domain.url}: ${error.message}`);
  ...
  async fallbackToAWS(domains, batchId) {
    console.log(`☁️ AWS fallback for batch ${batchId} (${domains.length} domains)`);
  ...
  async clearBatchHistory() {
    ...
    this.batchHistory = [];
    console.log(`🧹 Cleared ${clearedCount} batch history entries`);
    return clearedCount;
  }
