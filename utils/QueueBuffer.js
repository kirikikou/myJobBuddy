...
class QueueBuffer {
  ...
  async initialize() {
    ...
    console.log('🗂️ QueueBuffer initialized');
  }
  ...
  async addToQueue(request) {
    ...
    console.log(`📥 Request ${requestId} queued (estimated wait: ${Math.round(estimatedWaitTime)}min)`);
  ...
  async addToServerQueue(request) {
    ...
    console.log(`🖥️ Added to server queue (position: ${this.serverQueue.length})`);
  ...
  async processWithAWS(request) {
    ...
    console.log(`☁️ Delegated to AWS (queue position: ${this.awsQueue.length})`);
  ...
  async processServerQueue() {
    ...
    if (batchSize === 0) {
      console.log('⏳ Server at capacity, delaying queue processing');
      setTimeout(() => this.processServerQueue(), 5000);
      return;
    }
    
    const batch = this.serverQueue.splice(0, batchSize);
    console.log(`🔄 Processing server queue batch: ${batch.length} requests`);
    ...
    try {
      ...
      console.error(`❌ Server queue batch failed: ${error.message}`);
  ...
  completeRequest(requestId, result, processingTime) {
    ...
    if (request) {
      ...
      console.log(`✅ Request ${requestId} completed in ${Math.round(totalTime / 1000)}s`);
    }
  }
  ...
  async clearQueues() {
    ...
    console.log(`🧹 Cleared queues: ${serverCleared} server, ${awsCleared} aws, ${historyCleared} history`);
  ...
  async optimizeQueue() {
    ...
    console.log(`🔧 Queue optimization: ${this.serverQueue.length} requests, capacity: ${capacity}`);
    ...
    if (batchConfig.estimatedTime > parallelizationConfig.MAX_WAIT_TIME_MINUTES * 60 * 1000) {
      ...
      console.log(`⚡ Moved ${moved.length} requests to AWS queue for optimization`);
