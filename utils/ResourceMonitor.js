...
class ResourceMonitor {
  ...
  async initialize() {
    ...
    loggingService.service('ResourceMonitor','log',{ message: '🔧 ResourceMonitor initialized' });
  }
  ...
  async loadAverageStepTimes() {
    try {
      ...
      loggingService.service('ResourceMonitor', 'step-times-loaded', { count: this.averageStepTimes.size });
    } catch (error) {
      loggingService.error('Failed to load step times', { error: error.message });
    }
  }
  ...
  calculateOptimalParallel(domains) {
    if (!this.initialized) {
      loggingService.service('ResourceMonitor','log',{ message: '⚠️ ResourceMonitor not initialized, using default parallel count' });
  ...
  async updateStepTime(stepType, duration) {
    ...
    try {
      ...
      loggingService.error('Failed to save step times', { error: error.message });
    }
  }
