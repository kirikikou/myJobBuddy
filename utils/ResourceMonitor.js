...
class ResourceMonitor {
  ...
  async loadAverageStepTimes() {
    try {
      ...
      console.log(`📊 Loaded ${this.averageStepTimes.size} average step times`);
    } catch (error) {
      console.warn(`⚠️ Failed to load step times: ${error.message}`);
    }
  }
  ...
  calculateOptimalParallel(domains) {
    if (!this.initialized) {
      console.warn('⚠️ ResourceMonitor not initialized, using default parallel count');
  ...
  async updateStepTime(stepType, duration) {
    ...
    try {
      ...
      console.warn(`⚠️ Failed to save step times: ${error.message}`);
    }
  }
